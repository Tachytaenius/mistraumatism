		...
		if roundToShoot and not roundToShoot.fired then
			...
			for i = 1, count do
				...
				self:newProjectile({
					...
					range = roundType.projectileRangeStopsAtTargetPos and math.min(roundType.range, self:length(action.relativeX, action.relativeY)) or roundType.range,
...
function game:throwItem(entity, action, item, targetEntity)
	...
	self:newProjectile({
		...
		range = itemType.projectileRangeStopsAtTargetPos and math.min(itemType.range, self:length(action.relativeX, action.relativeY)) or itemType.range,
		entityHitRandomSeed = entityHitRandomSeed,
